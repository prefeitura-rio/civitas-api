...
if config.ENABLE_NEW_RADAR_LIST_ENDPOINT:
    @router_request(method="GET", router=router, path="/collection-points", response_model=List[LprCollectionPointOut])
    async def get_collection_points_list(
        user: Annotated[User, Depends(is_user)],
        request: Request,
    ):
        return await get_lpr_collection_points_positions()