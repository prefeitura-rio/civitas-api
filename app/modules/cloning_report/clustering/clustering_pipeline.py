...
class ClusteringPipeline:
    ...
    @staticmethod
    def build_daily_tracks(df_sus: pd.DataFrame, vmax_kmh: float = VMAX_KMH) -> Dict[str, Dict]:
        ...
        for day, day_data in prepared_df.groupby('_day', sort=True):
            daily_result = ClusteringPipeline._process_single_day(day, day_data, vmax_kmh)
            if daily_result:
    ...
    @staticmethod
    def _process_single_day(day: str, day_data: pd.DataFrame, vmax_kmh: float) -> Dict[str, Any]:
        ...
        tracks = TrackGenerator.generate_tracks(metadata['df_nodes'], metadata['labels'])
