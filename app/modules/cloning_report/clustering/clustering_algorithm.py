...
from ..utils import haversine_km, VMAX_KMH
...
class GreedyTemporalClustering:
    ...
    @staticmethod
    def cluster_nodes(df_nodes: pd.DataFrame, edges: List[Tuple[str, str, Dict]], vmax_kmh: float = VMAX_KMH) -> Dict[str, int]:
    ...
    @staticmethod
    def _find_best_label(node_id: str, opposite_map: Dict[str, str], labels: Dict[str, int], 
                        last_by_label: Dict[int, str], nodes_idx: pd.DataFrame, vmax_kmh: float) -> int:
        """Finds optimal label for node"""
        forced_label = GreedyTemporalClustering._get_forced_label(node_id, opposite_map, labels)
        ...
        for candidate in candidates:
            ...
            if GreedyTemporalClustering._is_better_choice(feasible, cost, best_feasible, best_cost):
                best_label, best_cost, best_feasible = candidate, cost, feasible
        
        return best_label if best_label is not None else 0
    
    @staticmethod
    def _get_forced_label(node_id: str, opposite_map: Dict[str, str], labels: Dict[str, int]) -> int:
    ...
    @staticmethod
    def _check_speed_feasibility(prev_id: str, cur_id: str, nodes_idx: pd.DataFrame, vmax_kmh: float) -> Tuple[bool, float]:
        ...
        dt_s = (pd.to_datetime(cur_node["datahora"]) - pd.to_datetime(prev_node["datahora"])).total_seconds()
        if dt_s <= 0:
            return True, 0.0
            
        distance_km = haversine_km(prev_node["latitude"], prev_node["longitude"], 
                                  cur_node["latitude"], cur_node["longitude"])
        speed = distance_km / (dt_s / 3600.0)
        return (speed <= vmax_kmh), speed
    
    @staticmethod
    def _is_better_choice(feasible: bool, cost: float, best_feasible: bool, best_cost: float) -> bool:
...
class SpatialKMeansClustering:
    ...
    @staticmethod
    def cluster_nodes(df_nodes: pd.DataFrame, edges: List[Tuple[str, str, Dict]], seed: int = 123) -> Dict[str, int]:
        ...
        return SpatialKMeansClustering._repair_with_local_search(initial_labels, edges, df_nodes)
    ...
    def _perform_kmeans(df_nodes: pd.DataFrame, seed: int) -> Dict[str, int]:
        ...
        return {node_id: int(label) for node_id, label in zip(df_nodes["node_id"], labels_array)}
    ...
    def _fallback_kmeans(coordinates: np.ndarray, seed: int) -> np.ndarray:
        ...
        centroids = coordinates[rng.choice(len(coordinates), size=2, replace=False)].astype(float)
    ...
    @staticmethod
    def _update_centroids(centroids: np.ndarray, coordinates: np.ndarray, labels: np.ndarray) -> None:
    ...
    @staticmethod
    def _repair_with_local_search(labels: Dict[str, int], edges: List[Tuple[str, str, Dict]], 
                                 df_nodes: pd.DataFrame) -> Dict[str, int]:
        ...
        while improved:
            ...
            for node_id in list(df_nodes["node_id"]):
                if SpatialKMeansClustering._try_flip_node(node_id, labels, edges, current_violations):
                    current_violations = SpatialKMeansClustering._count_violations(labels, edges)
                    improved = True
    ...
    @staticmethod
    def _count_violations(labels: Dict[str, int], edges: List[Tuple[str, str, Dict]]) -> int:
        """Counts constraint violations"""
        return sum(1 for u, v, _ in edges if labels.get(u, 0) == labels.get(v, 0))
    
    @staticmethod
    def _try_flip_node(node_id: str, labels: Dict[str, int], edges: List[Tuple[str, str, Dict]], 
                      current_violations: int) -> bool:
