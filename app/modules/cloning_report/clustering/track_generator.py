...
class TrackGenerator:
    ...
    @staticmethod
    def generate_tracks(df_nodes: pd.DataFrame, labels: Dict[str, int]) -> Dict[str, pd.DataFrame]:
    ...
    @staticmethod
    def _prepare_dataframe(df_nodes: pd.DataFrame, labels: Dict[str, int]) -> pd.DataFrame:
    ...
    def _format_timestamps(df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df.dropna(subset=['DataHora', 'latitude', 'longitude']).sort_values('DataHora').copy()
    ...
    def _add_location_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Sentido'] = df.get('logradouro', pd.Series(index=df.index, dtype='object')).map(
            TrackGenerator._extract_direction
        )
        return df
    ...
    def _extract_direction(location_str: str) -> str:
        ...
        if match:
            return match.group(1).strip().upper().replace('SENTIDO ', '').replace('SENT ', '')
        
        # Try SENTIDO/SENT patterns
        match = re.search(r'(?:SENTIDO|SENT\.?|SENT\s*:)\s*([A-ZÁÉÍÓÚÂÊÔÃÕÇ0-9\/\s]+)', 
                         location_str, flags=re.IGNORECASE)
        if match:
    ...
    def _remove_duplicates(df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df.drop_duplicates(subset=['DataHora', 'Latitude', 'Longitude'], keep='first')
