...
from app.modules.cloning_report.utils import haversine_km, VMAX_KMH, violations
...
class ClusteringValidator:
    ...
    @staticmethod
    def is_clusterizable(
        df_day: pd.DataFrame, vmax_kmh: float = VMAX_KMH
        ...
        feasibility_result = ClusteringValidator._check_clustering_feasibility(
            validation_result[1]
        )
        if not feasibility_result[0]:
            return feasibility_result

        return ClusteringValidator._select_clustering_strategy(
            feasibility_result[1], vmax_kmh
        )
    ...
    @staticmethod
    def _check_clustering_feasibility(
    ...
    @staticmethod
    def _select_clustering_strategy(
        prepared_data: dict[str, Any], vmax_kmh: float
    ) -> tuple[bool, dict[str, Any]]:
        """Evaluate and choose clustering approach"""
        return ClusteringValidator._evaluate_clustering_strategy(
            prepared_data, vmax_kmh
        )
    ...
    @staticmethod
    def _evaluate_clustering_strategy(
