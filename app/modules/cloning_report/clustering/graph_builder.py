...
class GraphBuilder:
    ...
    def create_nodes_and_edges(
        df_sus_day: pd.DataFrame,
    ) -> tuple[pd.DataFrame, list[tuple[str, str, dict]]]:
    ...
    def _prepare_timestamps(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df["Data_ts"] = pd.to_datetime(
            df.get("Data_ts", df.get("Data")), errors="coerce", utc=True
        )
        df["DataDestino"] = pd.to_datetime(
            df.get("DataDestino", df["Data_ts"]), errors="coerce", utc=True
        )
        return df
    ...
    def _create_node_pair(row: pd.Series, index: int) -> dict[str, Any]:
        ...
        return {
            "node_ids": (u, v),
            "nodes": [
                GraphBuilder._create_node_data(
                    u, row["Data_ts"], lat1, lon1, row, "origem"
                ),
                GraphBuilder._create_node_data(
                    v, row["DataDestino"], lat2, lon2, row, "destino"
                ),
            ],
        }
    ...
    def _create_node_data(
        node_id: str, timestamp, lat: float, lon: float, row: pd.Series, prefix: str
    ) -> dict[str, Any]:
    ...
    @staticmethod
    def _create_edge(
        row: pd.Series, node_pair: dict[str, Any]
