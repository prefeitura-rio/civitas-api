...
class KPIRenderer:
    ...
    def render_kpi_box(self, title, value, x, y, w=40, h=47, num_suspeitos=0, 
                      pad_x=5, pad_top=6, pad_bottom=6):
        """Render KPI box with title, value and icon"""
        self._draw_kpi_background(x, y, w, h)
        layout = self._calculate_kpi_layout(x, y, w, h, pad_x, pad_top, pad_bottom, title)
    ...
    def _get_icon_mapping(self):
        ...
        return {
            'Numero': 'icons/warning.png',
            'Dia': 'icons/calendar.png',
            'Turno': 'icons/clock.png',
            'Radar': 'icons/radar.png',
            'Par': 'icons/crescer.png',
            'Veloc': 'icons/car-speed.png',
            'OK': 'icons/ok.png',
    ...
    def _determine_icon_type(self, title, value, num_suspeitos, icon_map):
        ...
        if 'número total de registros' in t or 'numero total de registros' in t:
        ...
        elif 'número de registros suspeitos' in t or 'numero de registros suspeitos' in t:
            ...
            if isinstance(value, str) and 'REGISTROS SUSPEITOS' in value:
                return icon_map['Caution'] if 1 <= num_suspeitos <= 2 else icon_map['Warn']
            else:
                return icon_map['OK']
        return None
    ...
    def _draw_kpi_icon(self, icon_path, layout):
        ...
        if icon_path and os.path.exists(icon_path):
            icon_w = icon_h = 12
            self.pdf.image(icon_path, x=layout['inner_x'] + (layout['inner_w'] - icon_w) / 2, 
                          y=layout['cur_y'], w=icon_w, h=icon_h)
    ...
    def _draw_kpi_value(self, title, value, num_suspeitos, layout):
        ...
        if title == 'Detecções Suspeitas de Clonagem' and isinstance(value, str) and 'REGISTROS SUSPEITOS' in value:
