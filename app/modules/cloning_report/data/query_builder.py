...
class BigQueryQueryBuilder:
    ...
    # Your production SQL query (single definition)
    BASE_QUERY = """
    WITH ordered_positions AS (
      SELECT DISTINCT
        DATETIME(datahora, "{timezone}") AS datahora,
        placa,
        codcet,
        camera_latitude,
        camera_longitude,
      ...
      WHERE
        ...
        AND datahora >= TIMESTAMP('{start_date}', "{timezone}")
        AND datahora <= TIMESTAMP('{end_date}', "{timezone}")
    ...
    @classmethod
    def build_parametrized_query(cls, timezone: str = "America/Sao_Paulo") -> tuple[str, list]:
        """Build parametrized query for better performance and security"""
        # Convert BASE_QUERY to use @parameters instead of {format}
        parametrized_query = cls.BASE_QUERY.replace(
            '"{timezone}"', '@timezone'
        ).replace(
            "'{plate}'", '@plate'
        ).replace(
            ...
            '"localidade (codcet)"', '`localidade (codcet)`'
        )
    ...
    def estimate_query_cost(cls, params: QueryParameters) -> Dict[str, Any]:
        ...
        # Rough estimations based on query complexity
        estimated_rows_scanned = days_span * 10000  # Estimate based on daily volume
        estimated_bytes = estimated_rows_scanned * 200  # Rough bytes per row
        
        return {
            'estimated_days_span': days_span,
            'estimated_rows_scanned': estimated_rows_scanned,
            'estimated_bytes_processed': estimated_bytes,
            'complexity_score': 'HIGH' if days_span > 30 else 'MEDIUM' if days_span > 7 else 'LOW',
