...
class DailyProcessor:
    ...
    def _generate_daily_figures(suspicious_pairs: pd.DataFrame) -> List[Dict[str, str]]:
        ...
        import sys
        sys.path.append('.')
        from ..maps import render_daily_figures
        return render_daily_figures(suspicious_pairs)
    
    @staticmethod
    def create_daily_tables(suspicious_pairs: pd.DataFrame) -> Dict[str, Dict[str, pd.DataFrame]]:
    ...
    def _group_by_day(df: pd.DataFrame) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
        daily_tables = {}
        required_cols = ['Data', 'DataDestino', 'DataFormatada', 'Origem', 'Destino', 
                        'Km', 's', 'Km/h', 'latitude_1', 'longitude_1', 'latitude_2', 'longitude_2']
    ...
    @staticmethod
    def _extract_table_columns(group: pd.DataFrame, required_cols: List[str]) -> pd.DataFrame:
