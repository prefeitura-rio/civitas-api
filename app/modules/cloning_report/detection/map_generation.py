...
class MapGenerator:
    ...
    @staticmethod
    def generate_detection_map(suspicious_pairs: pd.DataFrame, all_detections: pd.DataFrame, 
                              speed_limit: float) -> str:
        """Generate main detection map and return HTML path"""
        html_path = MapGenerator._prepare_html_path()
        html_content = MapGenerator._create_map_content(suspicious_pairs, all_detections, speed_limit)
    ...
    @staticmethod
    def _create_map_content(suspicious_pairs: pd.DataFrame, all_detections: pd.DataFrame, 
                           speed_limit: float) -> str:
        """Create map HTML content"""
        # Import here to avoid circular dependency
        import sys
        sys.path.append('.')
        from ..maps import generate_map_clonagem
