...
class ParallelPairDetector:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        logger.info(f"ParallelPairDetector initialized with {self.max_workers} workers")
    ...
    def find_suspicious_pairs(self, df: pd.DataFrame, speed_limit: float) -> pd.DataFrame:
        ...
        logger.info(f"Starting parallel detection on {len(df)} records with {self.max_workers} processes")
        ...
        logger.info(f"Parallel detection completed: {len(all_pairs)} pairs found in {total_time:.2f}s")
        
        return pd.DataFrame(all_pairs)
    
    def _create_chunks(self, df: pd.DataFrame, speed_limit: float) -> List[DetectionChunk]:
        """Create overlapping chunks for parallel processing"""
        # Calculate optimal chunk size based on data size and worker count
        total_rows = len(df)
        optimal_chunk_size = max(50, total_rows // (self.max_workers * 2))  # Ensure minimum chunk size
        
        chunks = []
        chunk_id = 0
        
        for start_idx in range(0, total_rows, optimal_chunk_size):
            end_idx = min(start_idx + optimal_chunk_size + 1, total_rows)  # +1 for overlap
    ...
    def _process_chunks_parallel(self, chunks: List[DetectionChunk]) -> List[ChunkResult]:
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_chunk):
                ...
                try:
                    ...
                    logger.debug(f"Chunk {result.chunk_id}: {result.pairs_found} pairs in {result.processing_time:.2f}s")
    ...
    def _combine_chunk_results(self, chunk_results: List[ChunkResult]) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"Combined results: {len(all_pairs)} unique pairs from {len(chunk_results)} chunks")
        return all_pairs
...
def process_detection_chunk(chunk: DetectionChunk) -> ChunkResult:
    """
    Process a single detection chunk in a separate process.
...
class DetectionPerformanceTracker:
    ...
    def benchmark_detection_methods(self, df: pd.DataFrame, speed_limit: float) -> Dict[str, Any]:
        ...
        logger.info(f"Benchmark complete: {speedup:.2f}x speedup ({sequential_time:.2f}s → {parallel_time:.2f}s)")
