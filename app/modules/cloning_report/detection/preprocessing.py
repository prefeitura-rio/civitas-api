...
class DetectionPreprocessor:
    ...
    @staticmethod
    def _preprocess_dataframe(df: pd.DataFrame, dedup_same_loc_time: bool = True) -> pd.DataFrame:
    ...
    def _add_location_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df['logradouro (codcet)'] = df['logradouro'].astype(str) + ' (' + df['codcet'].astype(str) + ')'
        df['localidade (codcet)'] = df['localidade'].astype(str) + ' (' + df['codcet'].astype(str) + ')'
        return df
