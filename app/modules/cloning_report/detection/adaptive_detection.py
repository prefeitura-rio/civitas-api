...
class AdaptiveDetector:
    ...
    def _analyze_dataset_complexity(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if "datahora" in df.columns:
            ...
            time_span_hours = (
                df_time["datahora"].max() - df_time["datahora"].min()
            ).total_seconds() / 3600
        else:
            time_span_hours = 0
        ...
        complexity = {
            ...
            "complexity_score": self._calculate_complexity_score(
                num_records, coord_diversity, time_span_hours
            ),
        }
    ...
    def _calculate_complexity_score(
        self, num_records: int, coord_diversity: float, time_span: float
    ) -> float:
    ...
    def benchmark_and_cache(
        ...
        if dataset_key in self.performance_cache:
            logger.debug("Using cached performance data for dataset type")
            return self.performance_cache[dataset_key]

        logger.info("Benchmarking detection methods for dataset caching")
        ...
        logger.info(
            f"Cached performance: {performance_data['recommended_method']} recommended"
        )
...
class HighPerformanceDetector:
    ...
    def detect_with_optimization(
        self, df: pd.DataFrame, speed_limit: float, force_method: str | None = None
    ) -> dict[str, Any]:
        ...
        logger.info(
            f"Detection completed using {method_used} method: {len(result)} pairs in {total_time:.2f}s"
        )
