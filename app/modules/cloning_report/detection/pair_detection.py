...
class PairDetector:
    ...
    def find_suspicious_pairs(
        cls, df: pd.DataFrame, speed_limit: float
    ) -> pd.DataFrame:
    ...
    @classmethod
    def _analyze_pair(
        cls, detection_a: pd.Series, detection_b: pd.Series, speed_limit: float
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            if speed_kmh > speed_limit:
                return cls._create_pair_record(
                    detection_a, detection_b, distance_km, time_diff_seconds, speed_kmh
                )
    ...
    def _calculate_distance(detection_a: pd.Series, detection_b: pd.Series) -> float:
        ...
        return haversine_km(
            float(detection_a["latitude"]),
            float(detection_a["longitude"]),
            float(detection_b["latitude"]),
            float(detection_b["longitude"]),
        )
    ...
    def _calculate_time_difference(
        ...
        return (
            pd.to_datetime(detection_b["datahora"])
            - pd.to_datetime(detection_a["datahora"])
        ).total_seconds()
    ...
    def _create_pair_record(
        detection_a: pd.Series,
        detection_b: pd.Series,
        distance_km: float,
        time_seconds: float,
        speed_kmh: float,
    ) -> dict[str, Any]:
        ...
        base_record.update(
            PairDetector._create_location_fields(detection_a, detection_b)
        )
        base_record.update(
            PairDetector._create_measurement_fields(
                distance_km, time_seconds, speed_kmh
            )
        )
        base_record.update(
            PairDetector._create_coordinate_fields(detection_a, detection_b)
        )
        base_record.update(
            PairDetector._create_metadata_fields(detection_a, detection_b)
        )
        return base_record

    @staticmethod
    def _create_timestamp_fields(
        detection_a: pd.Series, detection_b: pd.Series
    ) -> dict[str, Any]:
    ...
    def _create_measurement_fields(
        distance_km: float, time_seconds: float, speed_kmh: float
    ) -> dict[str, Any]:
