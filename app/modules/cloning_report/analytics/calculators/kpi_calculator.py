...
class KpiCalculator:
    ...
    def _get_basic_metrics(sus_df: pd.DataFrame) -> dict[str, Any]:
        ...
        vel = pd.to_numeric(
            sus_df.get("Km/h", pd.Series([], dtype=float)), errors="coerce"
        )
        return {
    ...
    def _analyze_daily_patterns(sus_df: pd.DataFrame) -> dict[str, Any]:
        ...
        day = pd.to_datetime(
            sus_df.get("Data_ts", sus_df.get("Data")), errors="coerce", utc=True
        ).dt.strftime("%d/%m/%Y")
    ...
    def _analyze_hourly_patterns(sus_df: pd.DataFrame) -> dict[str, Any]:
        ...
        def _get_time_period(h):
            return (
                "Madrugada"
                if h < 6
                else ("Manhã" if h < 12 else ("Tarde" if h < 18 else "Noite"))
            )
