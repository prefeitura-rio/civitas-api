...
class NeighborhoodAnalyzer:
    ...
    def _group_and_sort_pairs(sus_df: pd.DataFrame, columns: list) -> pd.DataFrame:
        ...
        grouped = sus_df.groupby(columns, dropna=False).size().reset_index(name='Contagem')
        renamed = grouped.rename(columns={
            'bairro_origem': 'Bairro Origem',
            'bairro_destino': 'Bairro Destino'
        })
        return renamed.sort_values('Contagem', ascending=False, kind='stable').reset_index(drop=True)