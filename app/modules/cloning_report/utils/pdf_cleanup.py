        ...
        for pdf_file in self.reports_dir.glob("*.pdf"):
            try:
                ...
                if file_age > self.max_age_seconds:
                    logger.info(f"Deleting old PDF: {pdf_file.name} (age: {file_age/3600:.1f}h)")
                    pdf_file.unlink()
                    deleted_count += 1
                else:
                    logger.debug(f"Keeping PDF: {pdf_file.name} (age: {file_age/3600:.1f}h)")
    ...
    def get_pdf_info(self) -> List[dict]:
        ...
        for pdf_file in self.reports_dir.glob("*.pdf"):
            try:
                ...
                logger.error(f"Error getting info for {pdf_file.name}: {str(e)}")
...
_cleanup_service: Optional[PDFCleanupService] = None
