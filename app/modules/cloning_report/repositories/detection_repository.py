...
from ..domain.detection import Detection, SuspiciousPair
...
class DetectionRepository(ABC):
    ...
    @abstractmethod
    def find_by_plate_and_period(self, plate: str, start_date: datetime, end_date: datetime) -> List[Detection]:
...
class DetectionMapper:
    ...
    def dataframe_to_detections(df: pd.DataFrame) -> List[Detection]:
        ...
        for _, row in df.iterrows():
            detection = Detection(
                datetime=pd.to_datetime(row['datahora']),
                plate=row['placa'],
                equipment_code=row['codcet'],
                latitude=float(row['latitude']),
                longitude=float(row['longitude']),
                speed=float(row['velocidade']) if 'velocidade' in row and pd.notna(row['velocidade']) else None,
