...
from ..utils.formatting import format_timestamp, get_optional_field
...
class BaseLayer:
    ...
    def __init__(self, map_obj: folium.Map, name: str = "Todas as detecções", 
                 color: str = "#808080", show: bool = True):
    ...
    def _add_edge_if_valid(self, p1: Tuple[float, float], p2: Tuple[float, float], velocity: float) -> None:
        """Adiciona aresta se válida"""
        same_point = (round(p1[0], 6) == round(p2[0], 6)) and (round(p1[1], 6) == round(p2[1], 6))
        if not same_point:
            folium.PolyLine([p1, p2], color=self.color, weight=3, opacity=0.95, dash_array="5, 10").add_to(self.feature_group)
    ...
    def _add_markers(self, p1: Tuple[float, float], p2: Tuple[float, float], 
                    ...
                    local_destino: str, velocity: float) -> None:
        ...
        folium.Marker([p1[0], p1[1]], icon=folium.Icon(icon="car", prefix="fa", color="gray"),
                     tooltip=format_timestamp(t1), popup=popup_origem).add_to(self.feature_group)
        
        if not (same_point and same_second):
            folium.Marker([p2[0], p2[1]], icon=folium.Icon(icon="car", prefix="fa", color="gray"),
