...
class TrailsMapGenerator:
    ...
    def generate_trails_map(self, df_sus: pd.DataFrame, day: str, 
                           trails_tables_day: dict) -> Dict[str, Optional[str]]:
        ...
        return {
            'carro1': self._build_map_for_car('carro1', BLUE_LIGHT, trails_tables_day, key_to_xy, day),
            'carro2': self._build_map_for_car('carro2', BLUE_DARK, trails_tables_day, key_to_xy, day)
        }
    ...
    def _build_coordinate_mapping(self, day_data: pd.DataFrame) -> Dict[Tuple[str, str], Tuple[float, float]]:
        ...
        df_nodes['DataHora_str'] = dh.dt.tz_convert(None).dt.strftime('%d/%m/%Y %H:%M:%S')
        df_nodes['Local'] = df_nodes.get('logradouro', '').astype(str)
        
        return dict(
            zip(
                zip(df_nodes['DataHora_str'], df_nodes['Local']),
                zip(df_nodes['latitude'].astype(float), df_nodes['longitude'].astype(float)),
            )
        )
    
    def _build_map_for_car(self, car_key: str, color_hex: str, 
                          trails_tables_day: dict, key_to_xy: Dict[Tuple[str, str], Tuple[float, float]], 
    ...
    def _build_map_html_for_car(self, df_sus: pd.DataFrame, day: str, trails_tables_day: dict, car_key: str) -> Optional[str]:
    ...
    def _extract_coordinates(self, tbl: pd.DataFrame, 
                           key_to_xy: Dict[Tuple[str, str], Tuple[float, float]]) -> list:
        """Extrai coordenadas da tabela"""
        t = tbl.copy()
        t['DataHora'] = pd.to_datetime(t['DataHora'], errors='coerce', format='%Y-%m-%d %H:%M:%S')
    ...
    def _find_coordinate(self, row: pd.Series, 
                        key_to_xy: Dict[Tuple[str, str], Tuple[float, float]]) -> Optional[Tuple[float, float]]:
        """Encontra coordenada para uma linha"""
        xy = key_to_xy.get((row['DataHora_str'], row['Local']))
        if xy is None:
            xy = next((v for (kk, _loc), v in key_to_xy.items() if kk == row['DataHora_str']), None)
        return xy
    ...
    def _create_map(self, coords: list) -> folium.Map:
        ...
        return folium.Map(location=[lat_center, lon_center], zoom_start=13, 
                         tiles="CartoDB Positron", control_scale=True)
    
    def _add_markers(self, m: folium.Map, coords: list, tbl: pd.DataFrame, color_hex: str) -> None:
        """Adiciona marcadores ao mapa"""
        for idx, ((lat, lon), (_, rr)) in enumerate(zip(coords, tbl.iterrows()), start=1):
            self._add_single_marker(m, lat, lon, rr, color_hex, idx)
    
    def _add_single_marker(self, m: folium.Map, lat: float, lon: float, 
                          row: pd.Series, color_hex: str, idx: int) -> None:
        ...
        icon = BeautifyIcon(
            icon="arrow-down", icon_shape="marker", number=str(idx),
            text_color=text_color, border_color='rgba(0,0,0,.35)',
            background_color=color_hex, inner_icon_style='font-weight:700;font-size:15px;',
        )
        
        folium.Marker([lat, lon], icon=icon, tooltip=tooltip_txt, 
                     popup=folium.Popup(popup_txt, max_width=360)).add_to(m)
    
    def _fit_bounds(self, m: folium.Map, coords: list) -> None:
        """Aplica bounds ao mapa"""
        lat_min, lat_max = float(np.min([c[0] for c in coords])), float(np.max([c[0] for c in coords]))
        lon_min, lon_max = float(np.min([c[1] for c in coords])), float(np.max([c[1] for c in coords]))
        pad_lat = max((lat_max - lat_min) * 0.15, 0.01)
        pad_lon = max((lon_max - lon_min) * 0.15, 0.01)
        bounds = [[lat_min - pad_lat, lon_min - pad_lon], [lat_max + pad_lat, lon_max + pad_lon]]
        try: 
            m.fit_bounds(bounds)
        except Exception: 
            pass
    
    def _save_map(self, m: folium.Map, day: str, car_key: str) -> Optional[str]:
        """Salva mapa como PNG"""
        safe_day = pd.to_datetime(day, dayfirst=True).strftime("%Y-%m-%d")
        tmp_html = ensure_dir('temp_files') / f"trilha_{safe_day}_{car_key}.html"
        out_png = ensure_dir('figs') / f"trilha_{safe_day}_{car_key}.png"
        
        with open(tmp_html, 'w', encoding='utf-8') as f:
            f.write(m.get_root().render())
        
        try:
            take_html_screenshot(str(tmp_html), str(out_png), width=self.width, height=self.height)
            return str(out_png)
        finally:
            try: 
                os.remove(tmp_html)
