...
from app.modules.cloning_report.utils import BLUE_LIGHT, BLUE_DARK, haversine_km
...
class ClusteredPairsLayer:
    ...
    def add_to_map(
        self, df_pairs_day: pd.DataFrame, labels: dict[str, int] | None = None
    ) -> tuple[folium.FeatureGroup, set[tuple[float, float, pd.Timestamp]]]:
    ...
    def _add_pair_to_layer(
        self, row: pd.Series, index: int, labels: dict[str, int]
    ) -> None:
        ...
        self._add_markers(
            p1, p2, t1, t2, local_origem, local_destino, velocity, col1, col2
        )
    ...
    def _add_edge_if_valid(
        ...
        if not same_point:
            ...
            folium.PolyLine(
                [p1, p2], color=color, weight=3.5, opacity=0.95, dash_array="5, 10"
    ...
    def _add_markers(
        ...
        folium.Marker(
            [p1[0], p1[1]],
            icon=folium.Icon(icon="car", prefix="fa", color=col1),
        ...
        if col2 is not None:
            folium.Marker(
                [p2[0], p2[1]],
                icon=folium.Icon(icon="car", prefix="fa", color=col2),
