...
class OtherDetectionsLayer:
    ...
    def __init__(
        self,
        map_obj: folium.Map,
        name: str = "Outras detecções (cinza)",
        show: bool = False,
        round_dec: int = 5,
    ):
    ...
    def _prepare_endpoints(
        self, endpoints: set[tuple[float, float, pd.Timestamp]]
    ) -> set[tuple[float, float, pd.Timestamp]]:
        """Prepara endpoints para exclusão"""
        return {
            (round(float(lat), self.round_dec), round(float(lon), self.round_dec), ts)
            for (lat, lon, ts) in endpoints
            if not pd.isna(ts)
        }

    def _get_occupied_coordinates(
        self, endpoints: set[tuple[float, float, pd.Timestamp]]
    ...
    def _group_detections(
        self,
        df_day_all: pd.DataFrame,
        used_endpoints: set[tuple[float, float, pd.Timestamp]],
    ...
    def _add_bucket_markers(
        ...
        occupied_xy.add(
            (round(lat_final, self.round_dec), round(lon_final, self.round_dec))
        )
    ...
    def _extract_velocities(self, rows: list) -> list:
        ...
        for rr in rows:
            vv = pd.to_numeric(
                get_optional_field(rr, "velocidade", "Velocidade"), errors="coerce"
            )
    ...
    def _create_tooltip(self, label: str, horas_fmt: list) -> str:
        ...
        return (
            f"{label} - {horas_fmt[0]} – {horas_fmt[-1]} ({len(horas_fmt)} detecções)"
        )
    ...
    def _find_non_overlapping_position(
        self, rows: list, occupied_xy: set[tuple[float, float]]
    ) -> tuple[float, float]:
    ...
    def _meters_to_deg(
        self, lat_deg: float, dlat_m: float, dlon_m: float
    ) -> tuple[float, float]:
