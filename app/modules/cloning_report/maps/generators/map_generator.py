...
from ..layers.layer_control import add_layer_control
from ..utils.mapping import fit_bounds_to_data
...
class MapGenerator:
    ...
    def __init__(self, use_clusters: bool = True, vmax_kmh: float = VMAX_KMH, verbose: bool = False):
        self.use_clusters = use_clusters
        self.vmax_kmh = vmax_kmh
        self.verbose = verbose
    
    def generate_map_clonagem(self, df_pairs: pd.DataFrame, *, base_only: bool = False,
                            df_all: Optional[pd.DataFrame] = None, show_other_daily: bool = False,
                            include_non_sus_days: bool = False) -> str:
        ...
        bounds_lats, bounds_lons = processor.get_bounds_coordinates(df_all, show_other_daily, include_non_sus_days)
    ...
    def _create_empty_map(self) -> str:
        """Cria mapa vazio"""
        m = folium.Map(location=[-22.90, -43.20], zoom_start=11, tiles="CartoDB Positron", control_scale=True)
        add_layer_control(m, collapsed=False)
        return m.get_root().render()
    
    def _process_daily_layers(self, m: folium.Map, processor: DataProcessor, 
                             df_all: Optional[pd.DataFrame], show_other_daily: bool) -> None:
        ...
        for day in order:
            ...
            if show_other_daily and isinstance(df_all, pd.DataFrame) and not df_all.empty:
                self._add_other_daily_detections(m, df_all, day, endpoints)
    
    def _get_cluster_info(self, g: pd.DataFrame) -> Tuple[bool, Optional[Dict[str, int]]]:
        ...
        try:
            ...
            can_cluster, meta = ClusteringValidator.is_clusterizable(g, vmax_kmh=self.vmax_kmh)
    ...
    def _add_other_daily_detections(self, m: folium.Map, df_all: pd.DataFrame, 
                                   day: str, endpoints: Set[Tuple[float, float, pd.Timestamp]]) -> None:
        ...
        df_day_all = dfa[dfa["_day"] == day].dropna(subset=["latitude","longitude","datahora"]).copy()
        
        other_layer = OtherDetectionsLayer(m, f"{day} - Detecções não suspeitas", False)
        other_layer.add_to_map(df_day_all, endpoints)
    
    def _process_non_suspicious_days(self, m: folium.Map, processor: DataProcessor, 
                                    df_all: Optional[pd.DataFrame], include_non_sus_days: bool) -> None:
        ...
        extra_days = [d for d in sorted(days_all, key=lambda s: pd.to_datetime(s, dayfirst=True)) if d not in sus_days]
        
        for d in extra_days:
            gg = dfa[dfa["_day"] == d].dropna(subset=["latitude","longitude","datahora"]).copy()
