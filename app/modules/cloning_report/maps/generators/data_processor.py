...
class DataProcessor:
    ...
    def _add_day_strings(self) -> None:
        ...
        self.df_pairs["_day_str"] = self.df_pairs["_processed_ts"].dt.strftime(
            "%d/%m/%Y"
        )
    ...
    def get_ordered_days(self) -> list:
        ...
        order = (
            self.df_pairs.assign(
                __k=pd.to_datetime(
                    self.df_pairs["_day_str"], format="%d/%m/%Y", errors="coerce"
                )
            )
            .groupby("_day_str")["__k"]
            .min()
    ...
    def get_coordinates(self):
        ...
        all_lats = pd.concat(
            [self.df_pairs["latitude_1"], self.df_pairs["latitude_2"]],
            ignore_index=True,
        ).astype(float)
        all_lons = pd.concat(
            [self.df_pairs["longitude_1"], self.df_pairs["longitude_2"]],
            ignore_index=True,
        ).astype(float)
        mask = all_lats.notna() & all_lons.notna()
        return all_lats[mask], all_lons[mask]

    def get_bounds_coordinates(
        self,
        df_all=None,
        ...
        if isinstance(df_all, pd.DataFrame) and (
            show_other_daily or include_non_sus_days
        ):
            extra_lats = pd.to_numeric(df_all.get("latitude"), errors="coerce").dropna()
            extra_lons = pd.to_numeric(
                df_all.get("longitude"), errors="coerce"
            ).dropna()

        return pd.concat([pairs_lats, extra_lats], ignore_index=True), pd.concat(
            [pairs_lons, extra_lons], ignore_index=True
        )
    ...
    def get_center(self) -> list:
        ...
        return [
            float(pairs_lats.median()) if not pairs_lats.empty else -22.90,
