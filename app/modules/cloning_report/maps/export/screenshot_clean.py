"""Clean screenshot processor for FastAPI deployment"""
import os
import time
...
class CleanScreenshotProcessor:
    ...
    def process_screenshots(self, tasks: List[ScreenshotTask], 
                          task_id: str = "screenshot_batch") -> Dict[str, any]:
        ...
        try:
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit all tasks
                future_to_task = {executor.submit(process_screenshot_task, task): task 
                                for task in tasks}
                    ...
                    try:
                        ...
                        if success:
                            self.progress_tracker.screenshot_completed(
                                task_id, os.path.basename(task.png_path), processing_time
                            )
                        else:
        ...
        logger.info(f"Batch completed: {successful}/{len(results)} successful in {total_time:.2f}s")
    ...
    def _format_results(self, results: List[ScreenshotResult], 
        ...
        return {
            ...
            'summary': {
                ...
                'avg_time_per_screenshot': round(total_time / len(results), 2) if results else 0.0
            }
        }


# Convenience function for backward compatibility
def process_screenshots_clean(tasks: List[ScreenshotTask], 
                             max_workers: Optional[int] = None,
                             progress_callback: Optional[Callable[[TaskProgress], None]] = None) -> Dict[str, any]:
