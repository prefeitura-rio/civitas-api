...
from app.modules.cloning_report.maps.export.screenshot_clean import (
    CleanScreenshotProcessor,
    ScreenshotTask,
)
from app.modules.cloning_report.maps.export.screenshot_clean import ScreenshotConfig
...
class APIMapRenderer:
    ...
    def _add_all_map_types(
        self,
        html_tasks: list[dict[str, Any]],
        df_sus: pd.DataFrame,
        trails_tables: dict[str, Any],
    ):
    ...
    def _process_daily_groups(
        self, df: pd.DataFrame, daily_tasks: list[dict[str, Any]]
    ):
    ...
    def _process_daily_map(
        self, day: str, df: pd.DataFrame, daily_tasks: list[dict[str, Any]]
    ):
    ...
    def _process_trail_groups(
        self,
        df: pd.DataFrame,
        df_sus: pd.DataFrame,
        trails_tables: dict[str, Any],
        trail_tasks: list[dict[str, Any]],
    ):
        for day, _g in df.groupby(df["Data_ts"].dt.strftime("%d/%m/%Y"), sort=True):
    ...
    def _process_trail_day(
        ...
        for car_key in ["carro1", "carro2"]:
            self._process_trail_car(
                day, car_key, df_sus, trails_tables_day, trail_tasks
            )

    def _process_trail_car(
        self,
        day: str,
        car_key: str,
        df_sus: pd.DataFrame,
    ...
    def _save_trail_html(
        self, day: str, car_key: str, trail_html: str, trail_tasks: list[dict[str, Any]]
    ):
    ...
    def _process_results(
        ...
        if result["success"]:
            self._process_successful_results(
    ...
    def _process_successful_results(
        ...
        html_tasks: list[dict[str, Any]],
        result: dict[str, Any],
        daily_figures: list[dict[str, Any]],
        overall_map: str | None,
        trails_maps: dict[str, Any],
    ):
        for i, task_result in enumerate(result["results"]):
            if task_result["success"]:
                self._process_single_result(
                    html_tasks[i], task_result, daily_figures, overall_map, trails_maps
                )

    def _process_single_result(
        self,
        html_task: dict[str, Any],
